...
<div class="box box-info">
    <form id="form_edit" role="form" enctype="multipart/form-data" class="form-horizontal" action="{{ route('appointment.store') }}" method="post">
        @csrf
        @method('POST')
                ...
                <div class="col-6 card">
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="title" title="Título" placeholder="Título" value="{{ old('title') ?? '' }}">
                        </div>
                        <div class="form-group">
                            <label for="start_time" class="control-label">Empieza</label>
                            <input type="datetime-local" class="form-control" name="start_time" title="Empieza" placeholder="Empieza" value="{{ old('start_time') ?? \Carbon\carbon::now()->format('Y-m-d\TH:i') }}">
                        </div>
                        <div class="form-group">
                            <label for="finish_time" class="control-label">Termina</label>
                            <input type="datetime-local" class="form-control" name="finish_time" title="Termina" placeholder="Termina" value="{{ old('finish_time') ?? \Carbon\carbon::now()->addHours(1)->format('Y-m-d\TH:i') }}">
                        </div>
                        <div class="form-group">
                            <label for="comments" class="control-label">Comentarios</label>
                                <textarea rows="2" cols="50" type="text" class="form-control" name="comments" id="comments">{{ old('comments') ?? '' }}</textarea>
                        </div>
